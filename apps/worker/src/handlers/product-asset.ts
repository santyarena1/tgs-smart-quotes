import{db,type ProcessingJob}from'@tgs/database';import{removeBackground}from'@tgs/providers';import{loadMediaStorage,normalizeMediaUrl}from'@tgs/storage';
export async function removeProductAssetBackground(job:ProcessingJob){
 ...
 // Con timeout: sin él, una URL que no responde dejaba el job colgado para
 // siempre y la imagen quedaba en "procesando" sin explicación.
 const sourceUrl=normalizeMediaUrl(asset.sourceUrl)??asset.sourceUrl;
 let response:Response;
 try{response=await fetch(sourceUrl,{signal:AbortSignal.timeout(25000)});}
 catch(error){throw new Error(`No se pudo descargar la imagen de origen: ${error instanceof Error?error.message:'error de conexión'}`);}
 if(!response.ok)throw new Error(`No se pudo descargar la imagen de origen (HTTP ${response.status})`);
