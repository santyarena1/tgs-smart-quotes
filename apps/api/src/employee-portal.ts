...
import {audit,employeeBalance,isEmployeePortalEnabled,obligationPendingCents,requireEmployeeForUser} from './employees-service.js';
...
export class EmployeePortalController {
  ...
  @Get() async profile(@CurrentUser()u:RequestUser){const employee=await this.employee(u.id);const [balance,salary,pendingMovements,openObligations,pendingRequests]=await Promise.all([employeeBalance(employee.id),currentSalary(employee.id),db.movement.count({where:{employeeId:employee.id,status:'PENDING'}}),db.obligation.count({where:{employeeId:employee.id,status:'OPEN'}}),db.employeeRequest.count({where:{employeeId:employee.id,status:'PENDING_APPROVAL'}})]);const basic=await db.employee.findUnique({where:{id:employee.id},select:{id:true,fullName:true,docId:true,position:true,active:true,branch:{select:{id:true,name:true}}}});return jsonSafe({...basic,balanceCents:balance,currentSalary:salary,summary:{pendingMovements,openObligations,pendingRequests}});}

  @Get('movements') async movements(@CurrentUser()u:RequestUser,@Query(new ZodPipe(movementsQuerySchema))q:MovementsQuery){const employee=await this.employee(u.id);return jsonSafe({items:await db.movement.findMany({where:{employeeId:employee.id,status:q.status,kind:q.kind,direction:q.direction,occurredAt:q.period?periodDates(q.period):undefined},orderBy:[{occurredAt:'desc'},{createdAt:'desc'}]})});}

  @Get('obligations') async obligations(@CurrentUser()u:RequestUser){const employee=await this.employee(u.id);const items=await db.obligation.findMany({where:{employeeId:employee.id},select:{id:true,kind:true,direction:true,originalAmountCents:true,description:true,status:true,createdAt:true,installments:{select:{id:true,number:true,amountCents:true,period:true,status:true,paidCents:true},orderBy:{number:'asc'}}},orderBy:{createdAt:'desc'}});return jsonSafe({items:await Promise.all(items.map(async item=>({...item,pendingCents:await obligationPendingCents(db,item)})))});}
