...
const PRODUCT_DESCRIPTION_SYSTEM='Sos redactor comercial de The Gamer Shop, Argentina. Redactá en español una descripción breve (1 a 2 oraciones, máximo 220 caracteres) de un componente de PC gamer para mostrar debajo de su nombre en una ficha de producto. Tono profesional y directo, sin emojis, sin inventar especificaciones numéricas que no te dieron, sin repetir el nombre completo del producto tal cual.';
async function generateProductDescription(productName:string):Promise<string>{const settings=await db.aiSettings.findUniqueOrThrow({where:{id:'singleton'}});if(!settings.enabled)throw new BadRequestException('La IA está desactivada. Activala en Ajustes → IA.');const key=settings.apiKeyEncrypted?decryptSecret(settings.apiKeyEncrypted):process.env.OPENAI_API_KEY;const client=createAiClient({apiKey:key});if(!client)throw new BadRequestException('Falta la API key de OpenAI. Cargala en Ajustes → IA.');const extra=settings.productDescriptionPrompt?.trim();const systemPrompt=extra?`${PRODUCT_DESCRIPTION_SYSTEM}\n\nInstrucciones adicionales definidas por el negocio (respetalas siempre que no contradigan las reglas anteriores): ${extra}`:PRODUCT_DESCRIPTION_SYSTEM;let completion;try{completion=await client.chat.completions.create({model:settings.model??DEFAULT_AI_MODEL,max_tokens:150,messages:[{role:'system',content:systemPrompt},{role:'user',content:`Componente: ${productName}`}]});}catch(error){const info=describeOpenAiError(error);throw new BadGatewayException(info.message);}const text=completion.choices[0]?.message?.content?.trim();if(!text)throw new BadGatewayException('La IA no devolvió una respuesta.');return text;}
...
export class ExternalModuleController{
 ...
 @Post('quotes/:versionId/unpublish')async unpublish(@Param('versionId',new ZodPipe(idSchema))versionId:string,@CurrentUser()u:RequestUser){await quoteVersion(versionId);const old=await db.webPublication.findUnique({where:{quoteVersionId:versionId}});await postWordpress('unpublish',{externalId:versionId});// Se limpia tambien la `url`: al despublicar, la pagina deja de existir en
// WordPress, asi que conservarla dejaba el boton "Ver en la tienda"
// apuntando a un 404.
const next=await db.webPublication.upsert({where:{quoteVersionId:versionId},create:{quoteVersionId:versionId,status:'UNPUBLISHED'},update:{status:'UNPUBLISHED',lastError:null,url:null}});await db.$transaction(tx=>audit(tx,u.id,next.id,'UNPUBLISH',old,next,'WebPublication'));return jsonSafe(next);}
 ...
 @Get('quotes/:versionId/publish-preview')async publishPreview(@Param('versionId',new ZodPipe(idSchema))versionId:string){await quoteVersion(versionId);try{return jsonSafe(await buildPublishPayload(versionId));}catch(error){if(error instanceof WordpressPublishError)throw new BadRequestException(error.message);throw error;}}
 ...
 @Post('quotes/:versionId/enrich')async enrichQuote(@Param('versionId',new ZodPipe(idSchema))versionId:string,@CurrentUser()u:RequestUser){const version=await quoteVersion(versionId);if(!version.items.length)throw new BadRequestException('El presupuesto no tiene ítems');const items=version.items.map(item=>({name:item.frozenName,quantity:item.quantity}));const aiSettings=await db.aiSettings.findUniqueOrThrow({where:{id:'singleton'}});const {result,metadata}=await(await enrichmentService()).enrich({items},{entity:{entityType:'QuoteVersion',entityId:versionId}},aiSettings.pcDescriptionPrompt);const old=await db.quoteEnrichment.findUnique({where:{quoteVersionId:versionId}});const next=await db.$transaction(async tx=>{const saved=await tx.quoteEnrichment.upsert({where:{quoteVersionId:versionId},create:{quoteVersionId:versionId,descriptionHtml:result.descriptionHtml,gamesJson:result.games,programsJson:result.programs,compatibilityJson:result.compatibility},update:{descriptionHtml:result.descriptionHtml,gamesJson:result.games,programsJson:result.programs,compatibilityJson:result.compatibility}});await audit(tx,u.id,saved.id,'GENERATE',old,saved,'QuoteEnrichment');return saved;});return jsonSafe({...next,ai:{usedAi:metadata.usedAi,cacheHit:metadata.cacheHit,model:metadata.model}});}
