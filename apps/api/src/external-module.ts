 ...
 */
/**
 * "Cambios sin publicar": la tienda tiene una copia vieja si la versión
 * publicada no es la activa, o si el contenido (ítems/totales de la versión, o
 * título/miniatura/hero de la familia) se modificó después de la última
 * publicación. Un margen de 5 s cubre las escrituras del propio publicar.
 */
function hasUnpublishedChanges(publication:{status:string;publishedAt:Date|null;quoteVersionId:string}|null,active:{id:string;updatedAt:Date}|null,published:{id:string}|null,familyUpdatedAt:Date){if(!publication||publication.status!=='PUBLISHED'||!active||!published)return false;if(published.id!==active.id)return true;const since=(publication.publishedAt?.getTime()??0)+5000;return active.updatedAt.getTime()>since||familyUpdatedAt.getTime()>since;}
async function publicationView(familyId:string){const family=await db.quoteFamily.findUnique({where:{id:familyId},select:{activeVersion:true,updatedAt:true,webTitle:true,webTagline:true,versions:{select:{id:true,version:true,state:true,updatedAt:true}},webPublication:true,combos:{orderBy:{position:'asc'},select:{comboFamilyId:true}}}});if(!family)throw new NotFoundException('Presupuesto inexistente');const active=family.versions.find(v=>v.version===family.activeVersion)??family.versions[0]??null;const publication=family.webPublication;const published=publication?family.versions.find(v=>v.id===publication.quoteVersionId)??null:null;return{...(publication??{status:'DRAFT' as const,url:null,lastError:null,lastErrorAt:null,publishedAt:null,wpProductId:null,thumbnailUrl:null,quoteVersionId:null}),quoteFamilyId:familyId,webTitle:family.webTitle,webTagline:family.webTagline,comboFamilyIds:family.combos.map(c=>c.comboFamilyId),publishedVersionNumber:published?.version??null,activeVersionId:active?.id??null,activeVersionNumber:active?.version??null,isStale:hasUnpublishedChanges(publication,active,published,family.updatedAt)};}
...
export class ExternalModuleController{
 ...
 @Put('quote-families/:familyId/publish-settings')async updatePublishSettings(@Param('familyId',new ZodPipe(idSchema))familyId:string,@Body(new ZodPipe(quoteFamilyPublishSettingsSchema))body:QuoteFamilyPublishSettingsInput,@CurrentUser()u:RequestUser){const old=await db.quoteFamily.findUnique({where:{id:familyId}});if(!old)throw new NotFoundException('Presupuesto inexistente');const webTitle=body.webTitle!==undefined?body.webTitle?.trim()||null:undefined;const webTagline=body.webTagline!==undefined?body.webTagline?.trim()||null:undefined;// Editado a mano: el pipeline deja de proponerlo cuando cambian los componentes.
const editedByHand=(webTitle!==undefined&&webTitle!==(old.webTitle?.trim()||null))||(webTagline!==undefined&&webTagline!==(old.webTagline?.trim()||null));const data={...(body.autoRepublish!==undefined?{autoRepublish:body.autoRepublish}:{}),...(webTitle!==undefined?{webTitle}:{}),...(webTagline!==undefined?{webTagline}:{}),...(editedByHand?{webTitleAuto:false}:{}),...(body.comboDiscountBps!==undefined?{comboDiscountBps:body.comboDiscountBps}:{}),...(body.storeVisible!==undefined?{storeVisible:body.storeVisible}:{})};const next=await db.$transaction(async tx=>{const saved=await tx.quoteFamily.update({where:{id:familyId},data});if(body.comboFamilyIds!==undefined){// Solo combos reales, sin la propia familia; se reemplaza la lista completa en orden.
const ids=Array.from(new Set(body.comboFamilyIds.filter(id=>id!==familyId)));const combos=await tx.quoteFamily.findMany({where:{id:{in:ids},kind:'COMBO'},select:{id:true}});const valid=new Set(combos.map(c=>c.id));await tx.quoteFamilyCombo.deleteMany({where:{pcFamilyId:familyId}});let position=0;for(const id of ids){if(valid.has(id))await tx.quoteFamilyCombo.create({data:{pcFamilyId:familyId,comboFamilyId:id,position:position++}});}}await audit(tx,u.id,familyId,'UPDATE_PUBLISH_SETTINGS',old,{...saved,comboFamilyIds:body.comboFamilyIds},'QuoteFamily');return saved;});
// La lista de combos viaja en el payload: si la PC ya está en la tienda, se re-publica sola para que el modal la vea.
let republished=false;let republishError:string|null=null;if(body.comboFamilyIds!==undefined){const publication=await db.webPublication.findUnique({where:{quoteFamilyId:familyId},select:{status:true,quoteVersionId:true}});if(publication?.status==='PUBLISHED'&&publication.quoteVersionId){try{await publishQuote(familyId,{versionId:publication.quoteVersionId});republished=true;}catch(error){republishError=error instanceof Error?error.message:String(error);}}}return jsonSafe({...next,republished,republishError});}
 /** Combos cargados (kind=COMBO) para el selector "Combos para esta PC" del editor web. */
 @Get('combos')async combos(){const families=await db.quoteFamily.findMany({where:{kind:'COMBO'},orderBy:{createdAt:'desc'},select:{id:true,internalName:true,visibleNumber:true,webTitle:true,thumbnailUrl:true,comboDiscountBps:true,storeVisible:true,activeVersion:true,versions:{select:{version:true,totalSaleCents:true}},webPublication:{select:{status:true,url:true}}}});return jsonSafe(families.map(f=>{const active=f.versions.find(v=>v.version===f.activeVersion)??f.versions[0]??null;return{id:f.id,internalName:f.internalName,visibleNumber:f.visibleNumber,webTitle:f.webTitle,thumbnailUrl:f.thumbnailUrl,comboDiscountBps:f.comboDiscountBps,storeVisible:f.storeVisible,totalSaleCents:active?.totalSaleCents??null,publicationStatus:f.webPublication?.status??'DRAFT',url:f.webPublication?.url??null};}));}
 // Regenera la miniatura con IA a pedido (botón en Publicación Web), aunque ya
 // hubiera una: cada llamada es una imagen nueva.
 @Post('quote-families/:familyId/thumbnail/generate')async generateFamilyThumbnail(@Param('familyId',new ZodPipe(idSchema))familyId:string,@Body(new ZodPipe(z.object({regenerateCase:z.boolean().optional(),versionId:idSchema.nullable().optional()}).strict().optional()))body:{regenerateCase?:boolean;versionId?:string|null}|undefined,@CurrentUser()u:RequestUser){const family=await db.quoteFamily.findUnique({where:{id:familyId},select:{id:true,thumbnailUrl:true}});if(!family)throw new NotFoundException('Presupuesto inexistente');let versionId:string;try{versionId=await resolvePublishVersionId(familyId,body?.versionId??null);}catch(error){throw translateWordpressError(error);}try{const generated=await generateAiThumbnail({familyId,versionId,userId:u.id,regenerateCase:Boolean(body?.regenerateCase)});return{thumbnailUrl:generated.url,detail:generated.detail};}catch(error){if(error instanceof ThumbnailAiUnavailable)throw new BadRequestException(error.message);throw new BadGatewayException(error instanceof Error?error.message:'No se pudo generar la miniatura');}}
 @Post('quote-families/:familyId/thumbnail')async uploadFamilyThumbnail(@Param('familyId',new ZodPipe(idSchema))familyId:string,@Req()req:any,@CurrentUser()u:RequestUser){const family=await db.quoteFamily.findUnique({where:{id:familyId}});if(!family)throw new NotFoundException('Presupuesto inexistente');if(typeof req.file!=='function')throw new BadRequestException('Upload multipart no disponible en el servidor');const part=await req.file();if(!part)throw new BadRequestException('Selecciona una imagen');const mimetype=String(part.mimetype??'');if(!mimetype.startsWith('image/'))throw new BadRequestException('El archivo debe ser una imagen');const buffer=await part.toBuffer();if(!buffer.length)throw new BadRequestException('La imagen esta vacia');const key=`quote-thumbnails/${familyId}/${randomUUID()}.${ext(String(part.filename??''),mimetype)}`;const stored=await(await loadMediaStorage()).put(key,buffer,mimetype);const next=await db.$transaction(async tx=>{const saved=await tx.quoteFamily.update({where:{id:familyId},data:{thumbnailUrl:stored.url,thumbnailAuto:false,thumbnailInputsHash:null}});await audit(tx,u.id,familyId,'UPDATE_THUMBNAIL',{thumbnailUrl:family.thumbnailUrl},saved,'QuoteFamily');return saved;});return jsonSafe(next);}
  ...
  */
 @Get('publications')async publications(){const families=await db.quoteFamily.findMany({where:{OR:[{isBuiltPc:true},{kind:'COMBO'}]},select:{id:true,activeVersion:true,updatedAt:true,webTitle:true,webTagline:true,versions:{select:{id:true,version:true,updatedAt:true}},webPublication:true}});const out:Record<string,unknown>={};for(const family of families){const active=family.versions.find(v=>v.version===family.activeVersion)??family.versions[0]??null;const publication=family.webPublication;const published=publication?family.versions.find(v=>v.id===publication.quoteVersionId)??null:null;out[family.id]={...(publication??{status:'DRAFT',url:null,lastError:null,lastErrorAt:null,publishedAt:null,wpProductId:null,thumbnailUrl:null,quoteVersionId:null}),quoteFamilyId:family.id,webTitle:family.webTitle,webTagline:family.webTagline,publishedVersionNumber:published?.version??null,activeVersionId:active?.id??null,activeVersionNumber:active?.version??null,isStale:hasUnpublishedChanges(publication,active,published,family.updatedAt)};}return jsonSafe(out);}
