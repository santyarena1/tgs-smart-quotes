...
export class ExternalModuleController{
 ...
 @Post('quote-families/:familyId/thumbnail/generate')async generateFamilyThumbnail(@Param('familyId',new ZodPipe(idSchema))familyId:string,@CurrentUser()u:RequestUser){const family=await db.quoteFamily.findUnique({where:{id:familyId},select:{id:true,thumbnailUrl:true}});if(!family)throw new NotFoundException('Presupuesto inexistente');const versionId=await resolvePublishVersionId(familyId);try{const generated=await generateAiThumbnail({familyId,versionId,userId:u.id});return{thumbnailUrl:generated.url,detail:generated.detail};}catch(error){if(error instanceof ThumbnailAiUnavailable)throw new BadRequestException(error.message);throw new BadGatewayException(error instanceof Error?error.message:'No se pudo generar la miniatura');}}
