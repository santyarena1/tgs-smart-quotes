...
import {Alert,EmptyState,Field,Loading,PageHeader,Pill,Stat,StatStrip,errorMessage} from "./shared";
...
export function EmployeePortalView(){
  ...
  return <div className="page-stack">
    ...
    <div className="grid-2"><section className="panel"><h2 className="panel-title">Crear solicitud</h2>{ok?<Alert tone="ok">{ok}</Alert>:null}<form className="form-grid" onSubmit={submit}><Field label="Tipo"><select value={kind} onChange={e=>setKind(e.target.value)}><option value="ADVANCE">Adelanto</option><option value="REIMBURSEMENT">Reintegro</option><option value="SALARY_PAYMENT">Pago de sueldo</option><option value="MERCHANDISE">Mercadería</option><option value="CARD_CONSUMPTION">Consumo de tarjeta</option><option value="DEBT">Deuda</option></select></Field><Field label="Importe (ARS)"><input required inputMode="decimal" value={amount} onChange={e=>setAmount(e.target.value)}/></Field><Field label="Descripción (opcional)"><textarea value={description} onChange={e=>setDescription(e.target.value)}/></Field><button disabled={saving}>{saving?"Enviando…":"Enviar solicitud"}</button></form></section><section className="panel"><h2 className="panel-title">Mis solicitudes</h2>{requests.length?<div className="table-wrap"><table><thead><tr><th>Fecha</th><th>Tipo</th><th>Importe</th><th>Estado</th></tr></thead><tbody>{requests.map(item=><tr key={item.id}><td>{date(item.createdAt)}</td><td>{label(item.kind)}</td><td>{formatArs(item.amountCents)}</td><td><Pill>{label(item.status)}</Pill></td></tr>)}</tbody></table></div>:<EmptyState title="Todavía no hiciste solicitudes"/>}</section></div>
  </div>;
}
