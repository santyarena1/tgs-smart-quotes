...
export const thumbnailAiSettingsUpdateSchema=z.object({enabled:z.boolean().optional(),mode:z.enum(['LAYOUT','AI_SCENE']).optional(),gpuHeadlineThresholdCents:z.string().regex(/^d+$/).optional(),accentColor:z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),footer:z.array(thumbnailFooterBadgeSchema).max(4).optional(),caseAiMode:z.enum(['OFF','ALWAYS']).optional(),caseAiPrompt:z.string().max(3000).optional(),model:z.string().trim().min(1).max(60).optional(),quality:z.enum(['low','medium','high']).optional(),size:z.enum(['1024x1024','1536x1024','1024x1536']).optional(),prompt:z.string().max(6000).optional(),textMode:z.enum(['AI','OVERLAY','NONE']).optional(),textTemplate:z.string().max(400).optional(),overlayPosition:z.enum(['top','bottom']).optional(),overlayColor:z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),overlayFontSize:z.number().int().min(12).max(200).optional(),overlayFontFamily:z.string().trim().max(120).nullable().optional()}).strict();
