...
export const wordpressPublishItemSchema=z.object({name:z.string(),imageUrl:z.string().url().nullable(),specs:z.record(z.unknown())}).strict();
export const wordpressPublishPayloadSchema=z.object({externalId:idSchema,title:z.string(),slug:z.string(),priceListCents:z.string().regex(/^\d+$/),priceCashCents:z.string().regex(/^\d+$/),priceTransferCents:z.string().regex(/^\d+$/),installments:z.array(z.record(z.unknown())),items:z.array(wordpressPublishItemSchema),gallery:z.array(z.string().url()),model3dUrl:z.string().url().nullable(),thumbnailUrl:z.string().url().nullable(),descriptionHtml:z.string().nullable(),power:z.object({watts:z.number().int().nullable(),psu:z.number().int().nullable(),note:z.string().nullable()}).strict(),games:z.array(quoteEnrichmentGameSchema),compatibility:z.array(z.string())}).strict();
